...
import com.riozenc.api.util.File;
...
import com.riozenc.api.webapp.entity.UserInfoEntity;
        ...
        try {
            ...
            if ("500".equals(jsonMap.get("statusCode").toString())) {
                if ("结算户输入错误,该结算户下没有计量点".equals(jsonMap.get("message").toString())) {
                    ...
                    return JSON.toJSONString(yzcxBankDomain);
                }else if("该结算户号不存在或者该结算户号存在重复记录".equals(jsonMap.get("message").toString())) {
...
//        str2.setAMT10(returnMap.get("lastBalance").toString());
    ...
    public String reconciliation()  {
        ...
        //获取文件
        try {
            fileMap= File.readTxts(Global.getConfig("filePath") +"DZ"+dateStr+ ".txt", 1);
        ...
        for (Map<String, String> stringStringMap : fileMap) {
            List<String> list = new ArrayList<>();
            list = Arrays.asList(stringStringMap.get("4").split(","));
            for (String s : list) {
                set.add(s);
            }
        }
        ...
        //获取billing当日流水号
        Map<String, String> params = new HashMap<>();
        params.put("startPayDate","2020-12-22 00:00:00");
        params.put("endPayDate","2020-12-22 23:59:59");
        params.put("operator","999934");
        params.put("fChargeMode", "11");
        Map<String, Object> jsonMap = iBillingService.findChargeInfoForDz(params);
        List<ChargeDomain> returnMap = (List<ChargeDomain>) jsonMap.get("resultData");

        //对比
        for (ChargeDomain chargeDomain : returnMap) {
            if (!set.contains(chargeDomain.getArrearageId())) {
                needBack.add(chargeDomain.getId());
            }
        }

        logger.info("needBack:"+needBack);
        //少的缴费入账
        ...
        //多的抹账申请-审批（改状态）-抹账
        if(needBack.size()>0){
            Map<String, String> applyBackChargeBySettleParams = new HashMap<>();
            //chargeinfo ids
            applyBackChargeBySettleParams.put("ids",needBack.toString());
            applyBackChargeBySettleParams.put("eraseReason", "邮政储蓄银行对账");
            iBillingService.applyBackChargeBySettleForBank(applyBackChargeBySettleParams);

            Map<String,String> backChargeMap=new HashMap<String,String>();
            //chargeinfo ids
            backChargeMap.put("ids",needBack.toString());
            iBillingService.backCharge(backChargeMap);
        }
